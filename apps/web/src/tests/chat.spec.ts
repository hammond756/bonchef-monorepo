...
test.describe("Chat interface", () => {
    ...
    test("expands and focuses chat input on 'Beschrijf een recept'", async ({
    ...
    test("expands and focuses chat input on 'Importeer een recept'", async ({
        authenticatedPage: page,
    }) => {
        // Initial state check
        const input = page.getByTestId("chat-input")
    ...
    test("uploads image along with test and sends and show image in chat", async ({
        authenticatedPage: page,
    }) => {
        ...
        await page
            .getByTestId("chat-message-attached-image")
...
test.describe("Teaser interaction", () => {
    test("when a teaser is generated, it is displayed as a clickable component", async ({
        authenticatedPage: page,
    }) => {
        await page.click("[data-testid='surprise-me']")
        ...
        await expect(page.getByTestId("recipe-modal-save-recipe-button")).toBeVisible({
            timeout: 10000,
        })
