...
export function RecipeDetail({ variant, recipe, user }: RecipeDetailProps) {
    ...
    return (
        <RecipeStateProvider initialPortions={recipe.n_portions}>
            <RecipeStateInit />
...
// Little helper component to run a mount hook inside the RecipeStateProvider
function RecipeStateInit() {
    const restore = useRestoreRecipeStateFromUrl({ showToastOnFailure: true })
    useEffect(() => {
        restore()
    }, []) // only run once
    return null
