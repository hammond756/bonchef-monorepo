...
export function Chat() {
    const { isLoading, sendMessage, retryMessage, messages, setMessages } = useChat({
    ...
    function handleSurpriseAction() {
        handleSendMessage({
            message: `Verras me maar!`,
            webContent: [
                {
                    url: "https://random.com",
                    content: `Geef drie random recepten opties. ${uuidv4()}`,
                },
            ],
        })
    }
        ...
        setMessages((prev: ChatMessageData[]) => {
            ...
            return [
                ...prev.slice(0, -1),
                {
                    ...
                    botResponse: {
                        content:
                            errorMessage || "Sorry, er is iets mis gegaan. Probeer het opnieuw.",
                        payload: {
                            type: "text",
                        },
                    },
    ...
    function handleConversationComplete(
        ...
        _finalMessages: ChatMessageData[]
    ) {
        setTimeout(() => {
            mutateHistory()
        }, 1000)
    ...
    return (
        <div className="flex h-full flex-1 flex-col">
            ...
            <div
                ...
                className="fixed right-0 bottom-0 left-0 z-10 border-t border-gray-200 bg-white"
            >
                {messages.length === 0 && hasSelectedFilters ? (
                    <div className="p-4">
                        <Button
                            onClick={handleGenerateRecipe}
                            className="w-full rounded-lg bg-green-600 py-6 font-medium text-white transition-colors hover:bg-green-700"
                            disabled={isLoading}
                        >
                            Recept genereren met {selectedFiltersCount}{" "}
                            {selectedFiltersCount === 1 ? "filter" : "filters"}
                        </Button>
                    </div>
                ) : (
                    <ChatInput
