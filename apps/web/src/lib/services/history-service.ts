...
type ConversationMessage =
    ...
    | UserWebContentConversationMessage
    ...
    async toAgentHistory(messages: ConversationMessage[]): Promise<(HumanMessage | AIMessage)[]> {
        ...
        for (const message of messages) {
            if (message.type === "user") {
                ...
                message.payload.type === "teaser" &&
                "recipe" in message.payload &&
                message.payload.recipe
            ) {
                agentMessages.push(
                    new AIMessage(
                        this.templateTeaserWithRecipe(message.content, message.payload.recipe)
                    )
                )
            } else {
