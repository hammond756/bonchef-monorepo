...
export function useRecipeImportJobs() {
    ...
    const addJob = useCallback(
        async (type: z.infer<typeof RecipeImportSourceTypeEnum>, sourceData: string) => {
            ...
            try {
                ...
                if (session) {
                    ...
                    jobId = await startRecipeImportJob(type, sourceData)
                } else {
                    if (!onboardingSessionId) {
                        throw new Error(
                            "Onboarding session ID is required when user is not logged in"
                        )
                    }
                    jobId = await startRecipeImportJob(type, sourceData, onboardingSessionId)
                }
        ...
        [session, mutate, onboardingSessionId]
    )
