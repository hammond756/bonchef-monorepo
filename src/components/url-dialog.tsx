...
function validateUrl(url: string) {
    try {
        ...
        if (
            urlObject.hostname.includes("instagram") ||
            urlObject.hostname.includes("facebook") ||
            urlObject.hostname.includes("tiktok")
        ) {
            return {
            ...
            <form className="space-y-4" onSubmit={handleSubmit} ref={formRef}>
                ...
                <div className="flex justify-end space-x-2">
                    <Button
                        type="button"
                        variant="outline"
                        onClick={() => setExampleUrl(formRef as React.RefObject<HTMLFormElement>)}
                    >
                        Kies voor mij
                    </Button>
