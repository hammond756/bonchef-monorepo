...
export class CulinaryAgent {
    private smart: Runnable<
        BaseLanguageModelInput,
        LLMResponse,
        RunnableConfig<Record<string, unknown>>
    >
    private fast: Runnable<
        BaseLanguageModelInput,
        LLMResponse,
        RunnableConfig<Record<string, unknown>>
    >
    private intentModel: Runnable<
        BaseLanguageModelInput,
        IntentResponse,
        RunnableConfig<Record<string, unknown>>
    >
    ...
    private async initializePrompts() {
        this.prompts = {
            ...
            introduction: await langfuse.getPrompt("IntroductionIntent", undefined, {
                type: "text",
            }),
        }
    }
    ...
    private async detectIntent(history: (HumanMessage | AIMessage)[]): Promise<string> {
        const intentClassifierPrompt = await this.langfuse.getPrompt(
            "IntentClassifier",
            undefined,
            { type: "text" }
        )
        const messages = [new SystemMessage(intentClassifierPrompt.compile()), ...history]
    ...
    async processMessage(userInput: UserInput, conversationId: string) {
        ...
        const messages = [new SystemMessage(systemMessage), ...agentHistory]

        const historyCallback = new HistoryCallbackHandler(
            conversationId,
            lastMessageOrder,
            this.historyService
        )

        return model.streamEvents(messages, {
            version: "v2",
